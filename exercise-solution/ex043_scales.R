...
ggplot(data = world_2023, mapping = aes(x = e0, y = tfr, size = pop/1e6, colour = grp_develop)) +
  geom_point(shape = 1) +
  scale_size_continuous(range = c(1, 10), breaks = c(1, 10, 100, 1000)) +
...
ggplot(data = se_asia, mapping = aes(x = year, y = name, fill = net)) +
...
ggplot(data = se_asia, mapping = aes(x = year, y = e0, colour = name, linetype = name)) +
  geom_line() +
